...
T.P_low_beta = out_rel30(:,10); T.P_high_beta = out_rel30(:,11);
% Occipital
T.O_theta = out_rel30(:,13); T.O_alpha = out_rel30(:,14); T.O_beta = out_rel30(:,15);
T.O_low_beta = out_rel30(:,16); T.O_high_beta = out_rel30(:,17);

% === low-gamma (30–45Hz): default report log10 absolute power (exploratory) ===
T.F_low_gamma_logabs = log10(out_abs(:,6)  + eps);
T.P_low_gamma_logabs = log10(out_abs(:,12) + eps);
T.O_low_gamma_logabs = log10(out_abs(:,18) + eps);

% Optional: also export gamma relative power (uses 1-40Hz denominator) for supplementary checks
try
    doGammaRel = false;
    if isfield(cfg,'write_gamma_relative'); doGammaRel = logical(cfg.write_gamma_relative); end
catch
    doGammaRel = false;
end
if doGammaRel
    T.F_low_gamma = out_rel40(:,6);
    T.P_low_gamma = out_rel40(:,12);
    T.O_low_gamma = out_rel40(:,18);
end
...
for bi = 1:3
    ...
    if ~isempty(idx_view_low) && ~isempty(idx_view_high)
        ...
        rHigh = mean_rel_topo(idx_view_high, bandRange);
        dHL = rHigh - rLow;
        fig = figure('Name', sprintf('%s topoplot (ComplexityHigh - ComplexityLow)', upper(bandName)));
...
sceneTable = table( ...
    ...
    Tv.pair_id, ...
    Tv.dur_s, ...
    Tv.F_theta, Tv.F_alpha, Tv.F_beta, Tv.F_low_beta, Tv.F_high_beta, ...
    Tv.P_theta, Tv.P_alpha, Tv.P_beta, Tv.P_low_beta, Tv.P_high_beta, ...
    Tv.O_theta, Tv.O_alpha, Tv.O_beta, Tv.O_low_beta, Tv.O_high_beta, ...
    Tv.F_low_gamma_logabs, Tv.P_low_gamma_logabs, Tv.O_low_gamma_logabs, ...
    ...
    'VariableNames', { ...
        ...
        'F_theta', 'F_alpha', 'F_beta', 'F_low_beta', 'F_high_beta', ...
        'P_theta', 'P_alpha', 'P_beta', 'P_low_beta', 'P_high_beta', ...
        'O_theta', 'O_alpha', 'O_beta', 'O_low_beta', 'O_high_beta', ...
        'F_low_gamma_logabs', 'P_low_gamma_logabs', 'O_low_gamma_logabs', ...
        'F_TAR', 'F_TBR', 'F_BA', ...
        'P_TAR', 'P_TBR', 'P_BA', ...
        'O_TAR', 'O_TBR', 'O_BA' ...
    });
...
if any(high_hf)
    ...
    % Example: scene01_W45_C1 (scene_name preferred)
            ...
            for ii = 1:height(T)
                if isnan(T.scene_id(ii))
                    ...
                    if strlength(sn(ii))>0
                        label(ii) = "scene" + sprintf('%02d', T.scene_id(ii)) + "_" + sn(ii);
                    else
                        label(ii) = "scene" + sprintf('%02d', T.scene_id(ii)) + "_W" + w(ii) + "_" + cnd(ii);
                    end
                end
            end
        end
        ...
        if nargin >= 6 && ~isempty(designMap)
            ...
            for ii=1:height(T)
                ...
                elseif strlength(sn(ii))>0
                    ...
                    label(ii) = "scene" + sprintf('%02d', T.scene_id(ii));
                end
            end
        else
            for ii=1:height(T)
