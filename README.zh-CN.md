...
- `enable_unified_mirror_surface`（默认 `false`）：若与 curated 输出一起开启，则还会生成统一镜像入口层（`batch/tables`、`batch/figures`、`analysis/task_outputs`）。当前 `main` 默认也关闭它。
...
- **当前 `main` 的默认阅读顺序：** `batch/merged/` → `batch/qc/` → `batch/reports/` → `batch/analysis/` → `batch/paper/`。
- 当前 `main` 默认不再自动生成较重的 curated descriptive/inferential 层；只有在显式设置 `enable_curated_main_outputs=true` 时才会额外生成。
- 当前 `main` 默认也不生成 unified mirror 入口层；`batch/tables`、`batch/figures`、`analysis/task_outputs` 只有在显式设置 `enable_unified_mirror_surface=true` 时才预期出现。
- 单被试输出正在迁移到：
