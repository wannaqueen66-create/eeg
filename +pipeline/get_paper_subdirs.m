function [fp_paper, fp_tbl, fp_fig, fp_rep] = get_paper_subdirs(fp_sum, tag)
...
%   <fp_sum>/paper_fig or <fp_sum>/paper_fig_qc
...
if strcmpi(leaf,'batch')
    fp_paper = fullfile(fp_sum, 'paper', tag);
    fp_tbl = fullfile(fp_paper, 'tables');
    fp_fig = fullfile(fp_paper, 'figures');
    fp_rep = fullfile(fp_sum, 'paper', 'report');
else
    if strcmpi(tag,'qc')
