...
% Scatter plots with simple least-squares line
for i = 1:numel(EEG)
    for j = 1:numel(EYE)
        ...
        scatter(ax, x(keep), y(keep), 18, [0.20 0.45 0.80], 'filled', 'MarkerFaceAlpha',0.55);
        p = polyfit(x(keep), y(keep), 1);
        xx = linspace(min(x(keep)), max(x(keep)), 100);
        yy = polyval(p, xx);
        plot(ax, xx, yy, '-', 'Color',[0.85 0.25 0.20], 'LineWidth',1.6);
        try
            [r,pp] = corr(x(keep), y(keep), 'Type','Spearman', 'Rows','complete');
            ttl = sprintf('%s vs %s | r=%.2f, p=%.3g, n=%d', EEG{i}, EYE{j}, r, pp, sum(keep));
        catch
            ttl = sprintf('%s vs %s | n=%d', EEG{i}, EYE{j}, sum(keep));
        end
        title(ax, ttl, 'Interpreter','none');
        xlabel(ax, strrep(EYE{j}, '_', '\_'), 'Interpreter','tex');
        ylabel(ax, strrep(EEG{i}, '_', '\_'), 'Interpreter','tex');
...
if ismember('eye_qc_needs_review', AllSceneEye.Properties.VariableNames)
    ...
    for i = 1:numel(EEG)
        ...
        fig = figure('Visible','off','Color','w');
        ax = axes(fig); hold(ax,'on');
        cols = [0.30 0.65 0.35; 0.85 0.35 0.25];
        for gi = 0:1
            idx = keep & g==gi;
            yy = y(idx);
            xx = repmat(gi+1, numel(yy), 1) + (rand(size(yy))-0.5)*0.18;
            try
                boxchart(ax, repmat(gi+1, numel(yy),1), yy, 'BoxWidth',0.28, 'MarkerStyle','none', 'BoxFaceColor', cols(gi+1,:), 'BoxFaceAlpha',0.20);
            catch
            end
            scatter(ax, xx, yy, 18, cols(gi+1,:), 'filled', 'MarkerFaceAlpha',0.55);
        end
        set(ax, 'XTick', [1 2], 'XTickLabel', {'QC ok','Needs review'});
        ylabel(ax, strrep(EEG{i}, '_', '\_'), 'Interpreter','tex');
        title(ax, sprintf('%s by eye QC review flag', EEG{i}), 'Interpreter','none');
        grid(ax,'on'); box(ax,'off');
